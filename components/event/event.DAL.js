...
exports.uploadImage = async (req, res) => {
    try {
        ...
        // Ensure that the MongoDB client is properly initialized
        const mongo_util = require('../../utils/mongo.util'); // Update the path
        const mongo_client = await mongo_util.dbClient();
        ...
        if ((uploadFile.mimetype === 'image/jpeg') || (uploadFile.mimetype === 'image/png')) {
            uploadFile.mv(`${fileName}`, async (err) => {
                ...
                const bucket = new GridFSBucket(mongo_client, {
            ...
            return res.status(400).json(errorObject('File is of an invalid type', 400));
        }
    } catch (error) {
        throw error;
    }
